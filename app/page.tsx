...
export default async function Page() {
  ...
  const [catRes, transRes] = await Promise.all([
    supabase.from('cathegories').select('*').order('name', { ascending: true }),
    supabase.from('transactions').select('*, cathegories(name)').order('date', { ascending: false }).order('created_at', { ascending: false })
  ]);
      ...
      <div className="max-w-4xl mx-auto space-y-6 md:space-y-8">
        ...
        <div className="flex justify-between items-center px-2">
          <div className="flex items-center gap-2 md:gap-3">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-gradient-to-tr from-purple-600 to-blue-500 rounded-lg flex items-center justify-center shadow-lg shadow-purple-500/20">
        ...
        <header className="relative p-6 md:p-12 rounded-[2.5rem] bg-slate-900/50 border border-slate-800 overflow-hidden shadow-2xl backdrop-blur-xl">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-bold text-purple-400 uppercase tracking-widest mb-2">Total Saldo</p>
          ...
          <div className="grid grid-cols-2 gap-3 mt-8 relative z-10">
            <div className="p-4 rounded-2xl bg-emerald-500/5 border border-emerald-500/20">
              ...
              <p className="text-lg md:text-xl font-bold text-emerald-400 truncate font-mono">+{income.toLocaleString("id-ID")}</p>
            </div>
            <div className="p-4 rounded-2xl bg-rose-500/5 border border-rose-500/20">
              <p className="text-[10px] font-bold text-rose-500/70 uppercase mb-1 tracking-wide">Pengeluaran</p>
              <p className="text-lg md:text-xl font-bold text-rose-400 truncate font-mono">-{expense.toLocaleString("id-ID")}</p>
            </div>
          </div>
        </header>
